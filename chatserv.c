...
{
	// init variables
	int listenerFileDescriptor, incomingClientFileDescriptor, bytesRead, bytesSent;
	socklen_t clientAddressLength;
	struct sockaddr_in	serverAddress, clientAddress;
	char clientAddressString[INET_ADDRSTRLEN], clientMessageBuffer[messageBufferSize], broadcastMessageBuffer[broadcastBufferSize];
	fd_set	activeFileDescriptors, readyFileDescriptors;
	...
	//	Init Server Address (Protocol, IP address, port) bzero(&serverAddress, sizeof(serverAddress)); serverAddress.sin_family = AF_INET; serverAddress.sin_addr.s_addr = htons(INADDR_ANY); serverAddress.sin_port = htons(LISTEN_PORT);
